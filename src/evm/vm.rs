...
use revm_primitives::{Bytecode, LatestSpec, Env};
...
impl VMStateT for EVMState {
    fn get_s(&self, address: &EVMAddress) {
        if let Some(storage) = self.state.get(address) {
            println!("===> Storage:");
    ...
    VS: Default + VMStateT + 'static,
{
    ...
    fn execute_abi(
        ...
        // println!("evaluating data => {:?}", data);
        // // use cuda data
        // let mut data = if input.get_cuda_input().len() > 0 {
        //     // println!("cuda input: {:02x?}", input.get_cuda_input());
        //     Bytes::from(input.get_cuda_input())
        // } else {
        //     // println!("abi input=>");
        //     let abi = input.to_bytes();
        //     // println!("abi input: {:02x?}\n ==============", abi);

        //     if abi.len() > 0 {
        //         Bytes::from(abi)

        //     } else {
        //         // println!("direct input: {:02x?}", input.get_direct_data());
        //         Bytes::from(input.get_direct_data())
        //     }
        // };
        ...
        let mut r = if is_step {
            let tmp_post_exec = vm_state.post_execution.pop();
            if tmp_post_exec.is_none() {
            ...
            let mut post_exec = tmp_post_exec.unwrap().clone();
        ...
        // println!("return value: {:?}", hex::encode(r.output.to_vec()));
    ...
    VS: VMStateT + Default + 'static,
{
    ...
    #[cfg(not(feature = "flashloan_v2"))]
    fn execute(&mut self, input: &I, state: &mut S) -> ExecutionResult<EVMAddress, EVMAddress, VS, Vec<u8>> {
...
mod tests {
    ...
    use crate::evm::types::{EVMFuzzState, EVMU256, generate_random_address};
